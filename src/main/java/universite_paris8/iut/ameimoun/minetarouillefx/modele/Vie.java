...
public class Vie {
    ...
    private DoubleProperty vieActuelle; // Non-final car elle est réassignée dans setVieActuelle (attention à ça si c'est voulu)
    private final List<Runnable> damageTakenCallbacks;
    private final BooleanProperty isTakingDamage; // Propriété pour indiquer si le joueur subit activement des dégâts
    ...
    public Vie(double vieMaxInitiale) {
        ...
        this.isTakingDamage = new SimpleBooleanProperty(false); // Initialisé à faux par défaut

        // Listener pour déclencher les callbacks de "dégâts pris" uniquement si la vie diminue
